...
s2 = 'city'
agg_flows = []
all_areas = pd.unique(mobility_va.County.values)
for area in all_areas:
    area_df = mobility_va.loc[mobility_va['County']==area]
    area_dic = {}
    area_dic['FIPS'] = '051'+ str(int(area_df.FIPS.values[0])).zfill(3)
    if area.endswith(s2):
...
mob_va_cleaned = pd.DataFrame.from_dict(agg_flows)
#################################################################
